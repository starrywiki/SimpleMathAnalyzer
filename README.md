...
### 随机测试
测试数据的构造见`exam.h`\
请注意，默认会开启随机测试，随机测试仅包含词法分析和语法分析，等性判断需之后手动输入表达式进行测试\
### 交互逻辑
交互逻辑为随机测试完通过输入1或者2进入单个/两个表达式的词法分析和语法分析，输入两个表达式会在其上增加等性判断的逻辑\
交互示意：
```bash
============================================
...
```
如果需要关闭随机测试仅手动测试，请注释掉`main.cpp`中最开始的代码`#define ENABLE_RANDOM_TEST`
